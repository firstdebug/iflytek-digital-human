...
// 2. 从 Node 后端取得公开配置和一次性签名 URL
const [{ signedUrl }, config] = await Promise.all([
  fetch('/api/avatar-auth').then((response) => response.json()),
...
**不要在前端硬编码 apiSecret**:
```javascript
// apiSecret 只在 Node 后端环境变量中，前端只传 signedUrl
avatar.setApiInfo({
  appId: 'xxx',
  sceneId: 'xxx',
  signedUrl: await fetch('/api/avatar-auth').then(r => r.json()).then(r => r.signedUrl)
});
```
