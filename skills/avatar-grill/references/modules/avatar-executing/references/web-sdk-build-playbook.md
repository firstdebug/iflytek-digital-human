...
运行前主 Agent 按契约需要读取 `../../avatar-credentials/source-skill.md`、
`../../avatar-artifact-download/source-skill.md` 和
`../../avatar-network-debug/references/auth-verification.md`。它们只提供领域事实，不是独立 Skill 或新入口。
...
退出码 2 表示阻塞，3 表示 `needs_runtime_verification`，0 才表示 `ready_to_deliver`。无论调用上下文如何，构建、运行和浏览器证据都必须真实产生；模型不得手写运行证据、直接调用 gate 或直接调用 `telemetry.py complete`。
