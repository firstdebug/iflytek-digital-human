...
- 创建目标目录和下载 SDK 仅在契约 `resources.sdk.mode=download` 且 `allowed_mutations` 包含 `download_sdk` 时执行。
- 不覆盖用户已有的同名 SDK 文件；版本不确定时先报告现状，再选择新目录或由用户明确覆盖。
- 下载完成后以 `sdk-artifact.json` 记录相对入口、类型文件、版本和 SHA-256，供当前契约的环境检查和工程实现使用。
