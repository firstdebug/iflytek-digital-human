...
- 执行契约要求新增可单测业务逻辑时先写失败测试；SDK 真机部分使用平台门禁和设备证据。
- `avatar-verification`：承接无法单测的运行时/真机验证，与本资料 互补（单测管逻辑，verification 管跑起来）。
- `avatar-troubleshoot`：定位到具体逻辑 bug 后，用本资料 的"复现测试"锁定并防回归。
