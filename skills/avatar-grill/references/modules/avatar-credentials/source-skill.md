...
**WebSocket 地址来源**: `${CLAUDE_PLUGIN_ROOT}/tools/platform_endpoints.py`
...
| 3. 安全写入 | 平台查询结果直接写入 `.env`，密钥不进入对话 | `references/interactive-guide.md` |
...
# 插件根目录示例：宿主提供的插件根目录（必须包含 `tools/` 与 `config/`）
```
...
### Fallback
如果工具不存在或执行失败，保持当前契约为 `blocked_missing_credentials`，按 `references/interactive-guide.md` 记录恢复条件；不得降级为聊天中收集密钥。
...
- **固定端点门禁**：`WS_URL` 必须从 `tools/platform_endpoints.py` 读取并精确匹配其中的平台常量；
  host、path、query 任一不同均阻断。
...
- **门禁关系**: 当前执行契约读取本资料验证凭据；它不是独立入口。
- **后续资料**: SDK 产物读取 `avatar-artifact-download/source-skill.md`；网络问题读取 `avatar-network-debug` 的领域资料。
