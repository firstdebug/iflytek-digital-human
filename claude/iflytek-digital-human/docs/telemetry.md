...
该值首次计算后写入 `~/.claude/iflytek-digital-human/telemetry/anonymous_id.json`，之后只读该文件。
...
```
~/.claude/iflytek-digital-human/telemetry/
...
| 用户/模型行为 | Hook | 识别方式 |
|---|---|---|
| 输入 `/iflytek-digital-human:xxx` | `UserPromptSubmit` | prompt 文本正则 |
...
第三条是主路径 —— iflytek-digital-human 的多数 skill 未注册为可发现 skill，
