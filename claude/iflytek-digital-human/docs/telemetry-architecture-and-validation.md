...
文档日期：2026-08-24
适用范围：Claude Code `iflytek-digital-human` Skill telemetry、客户端本地队列、Zhisheng 后端接收与 MySQL 落库
...
| 能力 | 主要函数 | 具体行为 | 对上报的影响 |
|---|---|---|---|
| 路径和版本 | `TELEMETRY_DIR`、`STATE_PATH`、`CONSENT_PATH`、`NOTICE_VERSION`、`SCHEMA_VERSION` | 固定本地数据目录 `~/.claude/iflytek-digital-human/telemetry`，统一 state、授权和当前会话文件位置 | 所有模块读写同一份 JSON 状态，不会因为调用者不同而产生多份状态 |
...
{
  "decision": "block",
  "reason": "iflytek-digital-human 最终回答门禁未通过 ...",
...
```text
~/.claude/iflytek-digital-human/telemetry/state.json
```
...
| 方案 2：Webhook + JSON 状态机 | `~/.claude/iflytek-digital-human/telemetry/state.json` 保存 workflow、invocation、revision 和 upload 状态 | `uploader.py` 读取 JSON，直接 POST Webhook；ACK 按 ID + revision 更新状态 | **当前实现** | 满足当前单机、低吞吐、可读状态和有限离线重试需求 |
