...
**T0: 用户输入提示词**
```
用户输入: /iflytek-digital-human:avatar-verification 做 Android SDK
...
**T1: 模型调用 Skill 工具**
```
模型操作: Skill('iflytek-digital-human:avatar-verification')
触发: PreToolUse hook
```
...
**安装：**
```bash
/skill add iflytek-digital-human
```
...
- hook 自动触发、自动落本地库
- **无需安装任何依赖**（Python 标准库）
- 数据存储在 `~/.claude/iflytek-digital-human/telemetry/telemetry.db`
