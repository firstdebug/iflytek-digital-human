#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Claude hooks for avatar-platform JSON-state telemetry.
...
_TM_DIR = os.path.join(os.path.expanduser('~'), '.claude', 'avatar-platform',
                       'telemetry')
...
SLASH_RE = re.compile(r'/avatar-platform:([a-z0-9][a-z0-9-]*)', re.I)
PATHY_RE = re.compile(
    r'[A-Za-z]:[\\/][^\s"\']*|~?/[^\s"\']{3,}|https?://\S+|`[^`]*`', re.I)

ENTRY_SKILLS = {'avatar-workflow-entry', 'avatar-platform'}
TYPE_HINTS = {
