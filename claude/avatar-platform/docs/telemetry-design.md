...
| `skill_name` | varchar(128) | ✓ | skill 全名 | 如 `avatar-workflow-entry` / `avatar-web-template`。<br>带项目前缀：`avatar-platform:avatar-xxx` |
| `anonymous_id` | varchar(64) | ✓ | 匿名设备标识 | 冗余自 workflow，便于按设备聚合调用 |
| `source` | varchar(32) | ✓ | 调用来源 | `slash` — 用户在输入框敲 `/avatar-platform:xxx`<br>`skill_tool` — 模型调用 Skill 工具<br>`read` — Read 了 `skill.md` 文件 |
  ...
  "invocations": [
    {
      ...
      "workflowId": "wf_5efb68bf-7a22-4356-b4ad-0bc7bef0e871",
      "skillName": "avatar-platform:avatar-verification",
      "anonymousId": "anon_900bb23c914eecfa",
      "source": "skill_tool",
      "hitCount": 2,
...
└─────────────────────────────────────────────────────┘
                    ...
                    ▼
┌─────────────────────────────────────────────────────┐
│  ~/.claude/avatar-platform/telemetry/telemetry.db  │
...
**置 1 的条件**（任一命中）：
1. 用户敲了斜杠命令：`/avatar-platform:xxx`
...
```python
# 剥掉路径、URL、行内代码，避免假阳性
cleaned = re.sub(r'[A-Za-z]:[/\][^\s]*', ' ', prompt)  # C:\path\to\avatar-platform
...
- `cat ~/.claude/skills/avatar-platform/skill.md` — 读文件
- 通篇没提业务需求，只是在排查 bug
...
**插件目录的判定**：
```python
PLUGIN_ROOT = Path(__file__).resolve().parent.parent  # ~/.claude/skills/avatar-platform
...
**反例**（不置 1）：
- 改 `~/.claude/skills/avatar-platform/hooks/tracker.py` — 开发插件本身
...
| **噪声过滤** | 路径中的关键词不算 | `C:\avatar-platform\tools\x.py` | ✓ signal=0 |
...
| 路径中关键词假阳性 | 代码审查 | `python .../avatar-platform/tools/x.py` 被当成业务意图 | prompt 匹配前剥掉路径、URL、行内代码 |
...
# 强制绕过退避
python ~/.claude/skills/avatar-platform/tools/uploader.py --force

# 查看将要发送的报文
python ~/.claude/skills/avatar-platform/tools/uploader.py --dry-run
```
...
```bash
cat ~/.claude/avatar-platform/telemetry/debug.log
```
...
```bash
python ~/.claude/skills/avatar-platform/tools/telemetry.py stats
```
...
```bash
# 重置（清空所有数据）
python ~/.claude/skills/avatar-platform/tools/init_db.py --reset

# 删除已上传的旧数据（手动，未实现自动清理）
sqlite3 ~/.claude/avatar-platform/telemetry/telemetry.db \
