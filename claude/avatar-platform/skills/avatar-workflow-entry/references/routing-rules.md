...
语音能力扩展:
  ...
  route_to: 先执行语音确认门禁（AskUserQuestion 确认能力 + 交互形态），确认后 voice-interact / avatar-permissions-setup
  priority: medium
...
- "用 Android SDK 做虚拟客服"已经明确 SDK 自建，直接进入 `avatar-brainstorming`，并按
  `../../shared/delivery-modes.md` 确认 `workflow_mode`。
- **"给现有项目加语音交互"即使工程和目标明确，也必须先确认是否加入语音能力和交互形态**
  （按住说话 / 点击开始停止 / 自动 VAD / 全双工），确认后才用 `voice-interact`。
  涉及多项能力或架构调整时还要进入 `avatar-brainstorming` 并选择 `quick` 或 `strict`。
- **用户只要求文本对话时不得自行添加语音功能**；未确认的能力写入 `excluded`。
- `integration-guides` 仅解释 SDK 结构，不作为 Android 或 Web 生产代码来源；真正构建工程必须读
...
新增语音能力必须单独确认。两个门禁的完整规则见 `../../shared/delivery-modes.md`。
