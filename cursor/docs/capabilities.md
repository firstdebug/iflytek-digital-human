...
**Web SDK**(`avatar-text-driver` / `avatar-voice-interact` / `avatar-full-duplex` 等 skill)

- 适用:浏览器端集成、H5 页面、桌面 Web 应用
- 能力:文本/语音驱动、NLP 交互、全双工、动作控制、透明背景、字幕

**Android SDK**(`avatar-toolchain` skill + Android 分支)

- 适用:原生 Android 应用
- 能力:与 Web SDK 相同的完整交互能力,适配移动端

**iOS SDK**(`avatar-toolchain` skill + iOS 分支)
...
- **avatar-toolchain**:工具链检查(按平台分 web/android/ios)
...
- **文本驱动**(`avatar-text-driver`):发送文本让数字人播报
- **文本交互**(`avatar-text-interact`):大模型对话(带知识库/函数调用)
- **音频驱动**(`avatar-audio-driver`):发送音频让数字人播报
- **语音交互**(`avatar-voice-interact`):ASR 识别 + NLP 对话 + TTS 播报
- **全双工**(`avatar-full-duplex`):实时打断、流式识别
- **动作控制**(`avatar-action-control`):触发预设动作(挥手/点头等)
- **字幕配置**(`avatar-subtitle-setup`):同步字幕显示
- **透明背景**(`avatar-transparent-bg`):抠像、叠加到自定义背景
...
- **avatar-integration-guides**:分平台集成指南(Web/Android/iOS)
