...
WORKFLOW_TYPES = (
    'sdk_integration', 'credentials_setup', 'web_template',
    'live_streaming', 'knowledge_base', 'webapi_protocol',
    'troubleshoot', 'model_config', 'config_authoring',
)
SKILL_NAME_RE = re.compile(r'^avatar-[a-z0-9-]{1,120}$')
...
def _find_invocation(state, workflow_id, skill_name):
...
def _record_invocation(state, workflow_id, skill_name):
    if _find_invocation(state, workflow_id, skill_name):
        return False
    stamp = now_iso()
...
def start_workflow(workflow_type=None, skill_name='avatar-workflow-entry',
                   project_dir=None, session_id=None):
    """Start one explicit lifecycle for hosts without Claude Code hooks."""
    if not is_enabled():
        return None, 'disabled'
    if workflow_type and workflow_type not in WORKFLOW_TYPES:
        return None, 'workflow_type_invalid'
    if not SKILL_NAME_RE.fullmatch(str(skill_name or '')):
        return None, 'skill_name_invalid'
    session_id = session_id or '{}-{}'.format(AGENT_NAME, uuid.uuid4().hex)
    cwd = str(Path(project_dir or os.getcwd()).resolve())
    changed = False
    with locked_state() as state:
        if state is None:
            return None, 'state_lock_timeout'
        ...
        if item and item.get('status') == 'in_progress':
            if workflow_type and not item.get('workflow_type'):
                changed = _bump(item, workflow_type=workflow_type) or changed
        else:
            if item:
                workflow_id = '{}_{}'.format(
                    workflow_id[:54], uuid.uuid4().hex[:8])
            stamp = now_iso()
            item = {
                'workflow_id': workflow_id,
        ...
        changed = (_record_invocation(
            state, workflow_id, skill_name) or changed)
    ...
    }, 'started' if changed else 'existing'


def record_skill_invocation(skill_name, workflow_type=None, workflow_id=None,
                            session_id=None):
    """Record one downstream Skill once for an explicit workflow."""
    if not is_enabled():
        return None, 'disabled'
    if workflow_type and workflow_type not in WORKFLOW_TYPES:
        return None, 'workflow_type_invalid'
    if not SKILL_NAME_RE.fullmatch(str(skill_name or '')):
        return None, 'skill_name_invalid'
    if not workflow_id:
        session_id = session_id or read_current_session()[0]
        if not session_id:
    ...
    with locked_state() as state:
        if state is None:
            return None, 'state_lock_timeout'
        resolved_id = workflow_id or active_workflow_id(state, session_id)
        item = _find_workflow(state, resolved_id)
        if not item:
            return None, 'no_workflow_row'
        if item.get('status') != 'in_progress':
            return None, 'workflow_not_in_progress'
        if workflow_type and not item.get('workflow_type'):
            changed = _bump(item, workflow_type=workflow_type) or changed
        changed = (_record_invocation(
            state, resolved_id, skill_name) or changed)
    _flush_after_change(changed)
    return {
        'workflowId': resolved_id,
        'skillName': skill_name,
        'recorded': changed,
    }, 'recorded' if changed else 'existing'
...
def main():
    ...
    start.add_argument('--type', dest='wf_type', choices=WORKFLOW_TYPES)
    start.add_argument('--skill', default='avatar-workflow-entry')
    start.add_argument('--project', default=None)
    start.add_argument('--session', default=None)
    invoke = sub.add_parser('invoke')
    invoke.add_argument('--skill', required=True)
    invoke.add_argument('--type', dest='wf_type', choices=WORKFLOW_TYPES)
    invoke.add_argument('--workflow', default=None)
    ...
    if args.cmd == 'start':
        result, info = start_workflow(
            args.wf_type, args.skill, args.project, args.session)
        print(json.dumps(result if result else {'status': 'skipped', 'reason': info},
                         ensure_ascii=False))
    elif args.cmd == 'invoke':
        result, info = record_skill_invocation(
            args.skill, args.wf_type, args.workflow, args.session)
        print(json.dumps(result if result else {'status': 'skipped', 'reason': info},
                         ensure_ascii=False))
