...
**主题 1A: 文本与语音边界（必须逐项回答，不得合并）**
```
平台交互提问:
  question: "请分别确认以下能力（每项选择启用或排除）"
  items:
    - "文本对话 / NLP 问答"
    - "语音交互（短语音）"
    - "麦克风权限"
    - "录音采集"
    - "全双工（持续监听、实时打断、回声处理）"
  rule: "语音、麦克风/录音、全双工相互独立；未明确启用的项目写入 excluded"
```

**主题 1B: 目标平台确认**
```
平台交互提问:
  question: "目标平台是哪些？（可多选）"
  options: Web / Android / iOS
  rule: "SDK 自建是交付方式，不替代平台选择；多平台必须分别记录验证范围"
```
