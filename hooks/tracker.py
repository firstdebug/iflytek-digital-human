#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Claude hooks for avatar-grill JSON-state telemetry.

Business facts are persisted before an uploader process is started. Any
failure is swallowed by main so telemetry can never block the agent.
"""
...
_TM_DIR = os.path.join(os.path.expanduser('~'), '.claude', 'avatar-grill',
                       'telemetry')
...
SLASH_RE = re.compile(r'/avatar-grill:([a-z0-9][a-z0-9-]*)', re.I)
PATHY_RE = re.compile(
    r'[A-Za-z]:[\\/][^\s"\']*|~?/[^\s"\']{3,}|https?://\S+|`[^`]*`', re.I)

ENTRY_SKILLS = {'avatar-grill'}
TYPE_HINTS = {}
PRIMARY_SKILLS = set()
CONTRACT_TYPE_MAP = {
    'sdk_build': 'sdk_integration',
    'sdk_extend': 'sdk_integration',
    'web_template': 'web_template',
    'live_streaming': 'live_streaming',
    'webapi': 'webapi_protocol',
    'credentials': 'credentials_setup',
    'resource_management': 'credentials_setup',
    'model_config': 'model_config',
    'knowledge_base': 'knowledge_base',
    'configuration': 'config_authoring',
    'permissions': 'config_authoring',
    'diagnose': 'troubleshoot',
    'verify': 'sdk_integration',
    'docs': 'documentation',
}
AVATAR_KEYWORDS = [
    '虚拟人', '数字人', '讯飞', 'xfyun', 'avatar', '智能客服', '语音交互',
    '文本驱动', '音频驱动', '动作控制', '发音人', '知识库', 'anchorId',
    'vcn', 'Web对话', 'H5通话', '大屏交互', '透明背景', '字幕', '全双工',
    'virtual human', 'digital human',
]
COMPLETION_METHODS = ('verification_flag', 'artifact', 'artifacts_file')
RESUME_WINDOW_MINUTES = 5
STALE_MINUTES = 30
RELEVANT_TOOLS = {'Skill', 'Read'}


def _plugin_skill_names():
    names = {item.lower() for item in ENTRY_SKILLS}
    skills_dir = os.path.join(CURRENT_PLUGIN_ROOT, 'skills')
    try:
        for entry in os.listdir(skills_dir):
            folder = os.path.join(skills_dir, entry)
            if not os.path.isdir(folder):
                continue
            if (os.path.isfile(os.path.join(folder, 'SKILL.md')) or
                    os.path.isfile(os.path.join(folder, 'skill.md'))):
...
def _find_resumable(state, anonymous_id, cwd):
    """Find a recent unfinished workflow matching anonymous id and cwd."""
    if not cwd:
        return None
    ...
    for item in state.get('workflows', []):
        if item.get('anonymous_id') != anonymous_id or item.get('cwd') != cwd:
            continue
        ordinary = (not item.get('completion_method') and
                    item.get('status') in ('interrupted', 'in_progress'))
        invalid_sdk = (item.get('status') == 'completed' and
        ...
        if (ordinary or invalid_sdk) and activity and activity > cutoff:
...
def _insert_workflow(state, workflow_id, anonymous_id, cwd,
                     workflow_type=None):
    from telemetry_common import now_iso, plugin_version
    stamp = now_iso()
    item = {
...
def ensure_workflow(state, workflow_id, anonymous_id, signal=False, cwd=None,
                    session_id=None):
    ...
    if item is None:
        if not signal:
            return None
        previous_id = None if terminal else _find_resumable(
            state, anonymous_id, cwd)
        if previous_id:
            ...
            return previous_id
        _insert_workflow(state, workflow_id, anonymous_id, cwd)
        set_active_workflow(state, session_id, workflow_id)
        return workflow_id
...
def maybe_set_type_from_contract(state, workflow_id):
    """Derive completion semantics from the confirmed Grill contract."""
    item = _workflow(state, workflow_id)
    if not item or item.get('workflow_type') or not item.get('cwd'):
        return
    try:
        from pathlib import Path
        contract = Path(item['cwd']) / '.avatar' / 'avatar-run-contract.json'
        data = json.loads(contract.read_text(encoding='utf-8'))
        kind = (data.get('task') or {}).get('kind')
        workflow_type = CONTRACT_TYPE_MAP.get(kind)
        if workflow_type:
            _touch_workflow(item, workflow_type=workflow_type)
...
def record_invocation(state, workflow_id, skill, anonymous_id, source):
...
def route_workflow_for_skill(state, workflow_id, session_id, skill,
                             anonymous_id, cwd):
    ...
    _insert_workflow(state, new_id, anonymous_id, item.get('cwd') or cwd,
                     workflow_type=target_type)
    set_active_workflow(state, session_id, new_id)
    return new_id


def handle_pre(state, payload, workflow_id, anonymous_id, session_id=None):
    tool = payload.get('tool_name') or ''
    tool_input = payload.get('tool_input') or {}
    if tool == 'Skill':
        skill = (tool_input.get('skill') or '').split(':')[-1].strip().lower()
        if skill and skill in PLUGIN_SKILLS:
            workflow_id = ensure_workflow(
                state, workflow_id, anonymous_id, True, payload.get('cwd'),
                session_id) or workflow_id
            workflow_id = route_workflow_for_skill(
                state, workflow_id, session_id, skill, anonymous_id,
                payload.get('cwd'))
            record_invocation(state, workflow_id, skill, anonymous_id,
                              'skill_tool')
        return workflow_id
    if tool == 'Read':
        ...
        if match:
            ...
            workflow_id = route_workflow_for_skill(
                state, workflow_id, session_id, skill, anonymous_id,
                payload.get('cwd'))
            record_invocation(state, workflow_id, skill, anonymous_id, 'read')
    return workflow_id
...
def handle_prompt(state, payload, workflow_id, anonymous_id, session_id=None):
    ...
    if slashes:
        ...
        for skill in slashes:
            ...
            workflow_id = route_workflow_for_skill(
                state, workflow_id, session_id, skill, anonymous_id,
                payload.get('cwd'))
            record_invocation(state, workflow_id, skill, anonymous_id, 'slash')
    else:
        low = PATHY_RE.sub(' ', prompt).lower()
        if any(keyword.lower() in low for keyword in AVATAR_KEYWORDS):
            workflow_id = ensure_workflow(
                state, workflow_id, anonymous_id, True, payload.get('cwd'),
                session_id) or workflow_id
    item = _workflow(state, workflow_id)
    if item:
        ...
        set_active_workflow(state, session_id, workflow_id)
        maybe_set_type_from_contract(state, workflow_id)
...
def main():
    ...
    from telemetry_common import (active_workflow_id, is_enabled, locked_state,
                                  resolve_anonymous_id, spawn_uploader,
    ...
    with locked_state() as state:
        if state is None:
            return
        anonymous_id = state.get('anonymous_id') or resolve_anonymous_id()
        state['anonymous_id'] = anonymous_id
        workflow_id = active_workflow_id(state, session_id)
        if action == 'pre':
            handle_pre(state, payload, workflow_id, anonymous_id, session_id)
        elif action == 'prompt':
            handle_prompt(state, payload, workflow_id, anonymous_id, session_id)
        elif action == 'stop':
            opportunistic_scan(state, workflow_id, payload)
        elif action == 'end':
    ...
    if pending:
        spawn_uploader(force=(action == 'end'))
