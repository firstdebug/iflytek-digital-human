...
**根目录：`<plugin-root>`**（当前已加载的 `avatar-platform@skills-dir` 插件目录，Codex 自动解析为真实路径）
...
**注册事实**：当前 `avatar-platform@skills-dir` 的业务 Skill 都由 Codex 注册为可发现组件，实际清单以当前目录中的 `skills/*/SKILL.md` 为准。
