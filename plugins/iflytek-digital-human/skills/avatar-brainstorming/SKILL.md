...
访谈必须把以下项目分别记录为 `enabled` 或 `excluded`：文本对话、语音交互、麦克风权限、录音采集、全双工（持续监听/打断）。目标平台必须明确写为 Web、Android、iOS 之一或多项；“SDK 自建”只表示交付方式，不是平台答案。
...
- **需要展开访谈问题（平台交互提问 模板）** → 读 `references/interview-templates.md`
  - 首次接入访谈 → 见 4.1
  - 功能扩展访谈 → 见 4.2
  - 配置调整访谈 → 见 4.3
...
| `references/interview-templates.md` | Phase 4 访谈主题与 平台交互提问 模板（首次接入/功能扩展/配置调整） |
...
进入下一阶段前确认：
- [ ] 平台已识别（或已通过 平台交互提问 明确）
- [ ] 环境门禁已通过或用户已知风险选择跳过
- [ ] 意图已分类，非集成任务已正确路由
- [ ] **Phase 4 访谈已完成（有 平台交互提问 调用记录，至少覆盖核心功能主题）**
